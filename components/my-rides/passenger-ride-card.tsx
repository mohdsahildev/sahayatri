...
export default function PassengerRideCard({
  ride,
  isPast = false,
  ...
  const showBoardingCue =
    !isPast &&
    ride.status !== "cancelled" &&
    ride.status !== "completed" &&
    ride.status !== "ended";

  async function handleMessageDriver(e: React.MouseEvent) {
    e.preventDefault();
    e.stopPropagation();

    if (!ride.driver || chatLoading) return;

    setChatLoading(true);
    setChatError("");

    try {
      const response = await getRideChat(ride._id, ride.driver);
      if (response?.data?.chat?._id) {
        router.push(`/chats/${response.data.chat._id}`);
      } else {
        setChatError("Unable to open conversation.");
      }
    } catch (err) {
      console.error("Failed to start chat with driver:", err);
    ...
    <article className="rounded-3xl border border-[#EAE6DF] bg-white p-6 shadow-xs transition hover:border-[#C8522E]/30 hover:shadow-md">
      ...
      <div className="mt-5 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex-1 space-y-4">
          ...
          {/* Meta Info & Boarding Cue */}
          <div className="space-y-2 pt-2">
            <div className="flex flex-wrap items-center gap-3 text-xs font-medium text-slate-600">
              <span className="inline-flex items-center gap-1.5">
                <span className="font-bold text-[#1E2022]">
                  {ride.bookedSeats || 1} {(ride.bookedSeats || 1) === 1 ? "seat" : "seats"}
                </span>
                <span>reserved</span>
              </span>
            ...
            {showBoardingCue && (
              <div className="pt-1">
                <Link
                  href={`/rides/${ride._id}`}
                  className="inline-flex items-center gap-1.5 rounded-lg border border-amber-200 bg-amber-50/80 px-2.5 py-1 text-[11px] font-semibold text-amber-900 transition hover:bg-amber-100"
                >
                  <KeyRound size={12} className="text-amber-700" />
                  <span>Boarding PIN · View in Ride Details</span>
                </Link>
              </div>
            )}
          </div>
        ...
        <div className="flex flex-col justify-between gap-4 border-t lg:border-t-0 lg:border-l border-[#EAE6DF] pt-4 lg:pt-0 lg:pl-6 lg:w-64 shrink-0">
          <div>
            ...
            <p className="font-sans text-2xl font-black tracking-tight text-[#1E2022]">
              ₹{ride.price}
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            {canMessageDriver && (
              <button
                ...
                className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-[#EAE6DF] bg-white px-4 py-2.5 font-sans text-xs font-bold text-[#1E2022] shadow-xs transition hover:border-[#1E2022] hover:bg-[#FAF8F5] active:scale-98 disabled:cursor-not-allowed disabled:opacity-50"
              >
                <MessageCircle size={14} className="text-slate-600" />
                <span>{chatLoading ? "Opening chat..." : "Message Driver"}</span>
              </button>
            )}
